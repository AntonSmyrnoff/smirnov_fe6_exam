    ...
    for (var i=0; i<7; i++) {
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          createGrid( data.hits[i].tags, randomItem.webformatURL ); //формируем сетку с изображениями

          count+=1;
          if (count===7) { isotopeLoad() }; // после формирования сетки с изображениями запускаем "плитку"
        },
      });
    }
  };
